...
impl Graph
{
	...
	pub fn draw_to<T: PlotCanvas>(&self, canvas: &mut T)
	{
		...
		let mut plot_arg = String::new();

		match &self.data
		{
			GraphStyles::Lines2d(data) =>
				{
					plot_arg.push_str("'-' with lines");
					canvas.add_plot_data(&parse_plot_data(data.iter().map(|x| x as &[f64]).collect()));
				},
			GraphStyles::Lines2dExpr(string) => plot_arg.push_str(string),
			GraphStyles::Dots2d(data) =>
				{
					plot_arg.push_str("'-' with dots");
					canvas.add_plot_data(&parse_plot_data(data.iter().map(|x| x as &[f64]).collect()));
				},
			GraphStyles::Dots2dExpr(string) => plot_arg.push_str(&format!("{} with dots", string)),
			GraphStyles::Points2d(data) =>
				{
					plot_arg.push_str("'-' with points");
					canvas.add_plot_data(&parse_plot_data(data.iter().map(|x| x as &[f64]).collect()));
				},
			GraphStyles::Points2dExpr(string) => plot_arg.push_str(&format!("{} with points", string)),
			GraphStyles::LinesPoints2d(data) =>
				{
					plot_arg.push_str("'-' with linespoints");
					canvas.add_plot_data(&parse_plot_data(data.iter().map(|x| x as &[f64]).collect()));
				},
			GraphStyles::LinesPoints2dExpr(string) => plot_arg.push_str(&format!("{} with linespoints", string)),
			GraphStyles::Impulses2d(data) =>
				{
					plot_arg.push_str("'-' with impulses");
					canvas.add_plot_data(&parse_plot_data(data.iter().map(|x| x as &[f64]).collect()));
				},
			GraphStyles::Impulses2dExpr(string) => plot_arg.push_str(&format!("{} with impulses", string)),
			GraphStyles::Vectors2d(data) =>
				{
					plot_arg.push_str("'-' with vectors");
					canvas.add_plot_data(&parse_plot_data(data.iter().map(|x| x as &[f64]).collect()));
				},
			GraphStyles::Labels2d(data) =>
				{
					plot_arg.push_str("'-' with labels");
			...
			GraphStyles::Steps(data) =>
				{
					plot_arg.push_str("'-' with steps");
					canvas.add_plot_data(&parse_plot_data(data.iter().map(|x| x as &[f64]).collect()));
				},
			GraphStyles::StepsExpr(string) => plot_arg.push_str(&format!("{} with steps", string)),
			GraphStyles::FSteps(data) =>
				{
					plot_arg.push_str("'-' with fsteps");
					canvas.add_plot_data(&parse_plot_data(data.iter().map(|x| x as &[f64]).collect()));
				},
			GraphStyles::FStepsExpr(string) => plot_arg.push_str(&format!("{} with fsteps", string)),
			GraphStyles::YErrorBars(data) =>
				{
					plot_arg.push_str("'-' with yerrorbars");
					canvas.add_plot_data(&parse_plot_data(data.iter().map(|x| x as &[f64]).collect()));
				},
			GraphStyles::XErrorBars(data) =>
				{
					plot_arg.push_str("'-' with xerrorbars");
					canvas.add_plot_data(&parse_plot_data(data.iter().map(|x| x as &[f64]).collect()));
				},
			GraphStyles::XYErrorBars(data) =>
				{
					plot_arg.push_str("'-' with xyerrorbars");
					canvas.add_plot_data(&parse_plot_data(data.iter().map(|x| x as &[f64]).collect()));
				},
			GraphStyles::YErrorLines(data) =>
				{
					plot_arg.push_str("'-' with yerrorlines");
					canvas.add_plot_data(&parse_plot_data(data.iter().map(|x| x as &[f64]).collect()));
				},
			GraphStyles::XErrorLines(data) =>
				{
					plot_arg.push_str("'-' with xerrorlines");
					canvas.add_plot_data(&parse_plot_data(data.iter().map(|x| x as &[f64]).collect()));
				},
			GraphStyles::XYErrorLines(data) =>
				{
					plot_arg.push_str("'-' with xyerrorlines");
					canvas.add_plot_data(&parse_plot_data(data.iter().map(|x| x as &[f64]).collect()));
				},
			GraphStyles::FinanceBars(data) =>
				{
					plot_arg.push_str("'-' with financebars");
					canvas.add_plot_data(&parse_plot_data(data.iter().map(|x| x as &[f64]).collect()));
				},
			...
			GraphStyles::BoxErrorBars(data) =>
				{
					plot_arg.push_str("'-' with boxerrorbars");
					canvas.add_plot_data(&parse_plot_data(data.iter().map(|x| x as &[f64]).collect()));
				},
			_ => unimplemented!(),
		}
		
		plot_arg = plot_arg.replace(" with", &format!(" axes {}{} with", self.axes.0.to_string(), self.axes.1.to_string()));
